<?php
class Zend_View_Helper_Menu extends Zend_View_Helper_Navigation_HelperAbstract
{
    /**
    ...
    public function htmlify(Zend_Navigation_Page $page)
    {
        ...
        if($page->attribs != null){
        	foreach($page->attribs as $k => $v){
        		if(!isset($attribs[$k])){
        			$attribs[$k] = $v;
        		}
        	}
        ...
        if(isset($attribs['data-toggle']) && $attribs['data-toggle'] == 'dropdown'){
        	return '<' . $element . $this->_htmlAttribs($attribs) . '>'
        			. $this->view->escape($label) . ' <b class="caret"></b>'
        			. '</' . $element . '>';
        }
        return '<' . $element . $this->_htmlAttribs($attribs) . '>'
                                   ...
                                   $addPageClassToLi)
    {
        ...
        foreach ($iterator as $page) {
            ...
            if ($depth > $prevDepth) {
                ...
                $this->skipPrefixForId();
				if($depth > 0){
					$html .= $myIndent . '<ul class="dropdown-menu"'
							. $this->_htmlAttribs($attribs)
							. '>'
									. self::EOL;
				}else{
            ...
            $liClass = '';
            if($depth == 0 && $page->pages){
            	$page->class = $page->class . ' dropdown';
            }
            if($depth > 0 && $page->pages){
            	$page->class = $page->class . ' dropdown-submenu';
            }
            ...
            if($depth == 0 && $page->pages){
            	$page->attribs = array('data-toggle' => 'dropdown');
	            $html .= $myIndent . '    <li' . $liClass . '>' . self::EOL
	                   . $myIndent . '        ' . $this->htmlify($page) . self::EOL;
            }elseif($depth > 0 && $page->pages){
            	$html .= $myIndent . '    <li' . $liClass . '>' . self::EOL
            	. $myIndent . '        ' . $this->htmlify($page) . self::EOL;
            }else{
            	$html .= $myIndent . '    <li' . $liClass . '>' . self::EOL
